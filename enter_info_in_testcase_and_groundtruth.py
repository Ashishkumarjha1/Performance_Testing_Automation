...
class AlarmSystemDatabase:
    ...
    def list_of_channels_from_testcase_table(self):
        try:

            cursor = self.connection.cursor()
            # SQL query to retrieve channels from the testcase table
            query = "SELECT test_case_name FROM testcase"
            cursor.execute(query)

            # Fetch all rows from the query
            results = cursor.fetchall()

            self.testcase_channel_list = [row[0] for row in results]
    ...
    def insert_into_testcase(self, data):
        ...
        if (data[0] not in self.testcase_channel_list):
            try:
